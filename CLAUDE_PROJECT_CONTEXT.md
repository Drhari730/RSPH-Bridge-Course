...
| **Phase 23** | **Found live, on the freshly-deployed Cloudflare Worker, right after the project owner ran `wrangler deploy` themselves**: asking Setu to compare sensitivity/specificity/PPV/NPV "in a table" rendered every formula as the literal text `@@MATH1@@` instead of a typeset equation — even though a standalone formula outside a table rendered fine. Root cause: in both `simpleMarkdownToHtml()` (chat) and `renderLessonContent()` (lesson body), math extraction ran before table/flow extraction, so a formula inside a table cell or flow step got baked into that block's pre-rendered HTML (stored in `tableBlocks`/`flowBlocks`) as an `@@MATH#@@` token. The final placeholder substitution replaced `@@MATH#@@` on the outer `html` skeleton *first*, then spliced in the table/flow HTML afterward — by then there was no more MATH pass left to reach the token now sitting inside that newly-inserted content, so it stayed literal. Fixed by reordering: TABLE and FLOW get re-inserted first, MATH runs last so it can reach formulas wherever they ended up. Verified live with the exact question that surfaced the bug. |
| **Phase 24** | **Flowchart audit of all 30 lessons**, requested explicitly by the project owner. Extracted every lesson's actual `data-explainer` key and its `EXPLAINERS[key].cap` scene captions (not just guessed from titles) and classified each: is the lesson's content genuinely a process/sequence, and does its current visual deliver a flow/staged treatment where that's warranted? Result: **26/30 already correctly matched** — either genuinely sequential content already given flow/staged treatment (prevention timeline, POSDCORB cycle, SBAR steps, exposure pathway, hypothesis decision tree, ethics review pathway, ABDM highway, atherosclerosis stages, coding pipeline) or non-sequential content correctly left as a model/matrix/distribution rather than being forced into a flowchart (iceberg model, SDOH rainbow, UHC cube, Herzberg's two scales, 2×2 screening table, normal-distribution curves, TNM staging, CONSORT/STROBE reference matrix, etc.). **One real gap found and fixed**: Day 14, "Outbreak Investigation & Integrated Disease Surveillance" — the existing explainer (`epidemic_curves`) only covered epidemic-curve-shape recognition (point-source/continuous/propagated), leaving the lesson's actual namesake content — the classic CDC/Gregg's 10-step outbreak investigation procedure — with no dedicated visual at all (confirmed the lesson's prose `content` field does cover both curve shapes and the 10 steps in text, so replacing the explainer doesn't lose the curve-shape material). Built a new `outbreak_steps` explainer: 5 scenes, cumulative-reveal 5-box pipeline (Steps 1-2 Prepare & Confirm → 3-4 Verify & Define → 5-6 Find & Describe → 7-8 Hypothesize & Test → 9-10 Control & Communicate) plus a callout card making the clinically important point that control measures don't wait for Step 10. **Lesson learned mid-build**: the first version of the 5-box SVG pipeline overflowed its own `viewBox` (last box's right edge fell outside the 720-unit width), silently clipping the 5th box — caught by jumping the `Explainer` instance directly to its final scene (`ex.go(4)`) rather than trusting a single screenshot mid-autoplay, which had made it look like only 4 boxes existed at all. Recomputed box/gap widths to fit 5×120px boxes + 4 gaps inside the 680px available space, re-verified all 5 boxes render fully, then re-ran the Phase-15-style integrity check (every lesson's `data-explainer` key resolves to an `EXPLAINERS` entry with matching `cap`/`dwell` lengths) across all 30 lessons before deploying. Two optional (not broken) enhancement candidates were flagged but intentionally left as-is per the owner's go-ahead on the audit findings: Day 4 (IPHS tiers — hierarchy is correct; a referral-pathway flow would be a nice-to-have, not a fix) and Day 10 (BMW segregation — categorized bin list is correct; a waste-sorting decision-flowchart would be a nice-to-have) and Day 26 (word building — structural template is correct; a "how to decode a term" step-by-step would be a nice-to-have). |
